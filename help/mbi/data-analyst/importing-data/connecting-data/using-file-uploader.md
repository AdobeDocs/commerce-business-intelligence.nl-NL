...
1. **De enige ondersteunde indeling voor het uploaden van bestanden is `CSV` of`comma separated values`**. Als u in Excel werkt, kunt u het bestand opslaan in `.csv` gebruiken.
...
Valuta-waarden moeten het gegevenstype hebben `Decimal Number` geselecteerd, en deze kolommen automatisch rond aan twee decimale plaatsen in uw Data Warehouse. Als u uw decimale getallen niet wilt afronden of als u een grotere precisie wilt hebben dan dit, selecteert u de optie `Non-Currency Decimal Number` datatype
...
1. Ga om te beginnen naar **[!UICONTROL Data** > **File Uploads]**.
...
1. Ga om te beginnen naar **[!UICONTROL Manage Data** > **File Uploads]**.
